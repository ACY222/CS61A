...
def balanced(m):
    ...
    """
    A mobile is balanced means that:
        1. The torque applied by its two arms are equal. Torque: the length of
        the left/right rod multiplied by the total mass hanging from that rod
    ...
    if is_planet(m):
        ...
        left_torque = length(left(m)) * total_mass(left(m))
        right_torque = length(right(m)) * total_mass(right(m))
        return left_torque == right_torque and balanced(left_end) and \
            balanced(right_end)

    # my method, it's wrong! I don't consider the second condition
    left_torque = length(left(m)) * total_mass(left(m))
    right_torque = length(right(m)) * total_mass(right(m))
...
def max_path_sum(t):
    ...
    # my solution
    def path_to_leaves(branch, dist):
        dist = dist + label(branch)
        if is_leaf(branch):
            dist_to_leaves.append(dist)
        for b in branches(branch):
            path_to_leaves(b, dist)

    dist_to_leaves = []
    path_to_leaves(t, 0)
    return max(dist_to_leaves)
...
# m[1] is left, m[2] is right, what about m[0]
def left(m):
