def divide(quotients, divisors):
    ...
    return {q: [d for d in divisors] for q in quotients if d % q == 0}
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    return sqrt((get_lat(city_a) - get_lat(city_b)) ** 2 + (get_lon(city_a) -
                        get_lon(city_b)) ** 2)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    dist_ac = distance(city_c, city_a)
    dist_bc = distance(city_c, city_b)
    if dist_ac < dist_bc:
